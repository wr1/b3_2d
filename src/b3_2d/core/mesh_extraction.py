...
def extract_airfoil_and_web_points(section_mesh: pv.PolyData) -> tuple:
    ...
    if bb_size(te) > 0.03 * bb_size(section):
        # Clean cell data to match n_cells or move to point_data if matches n_points
        # for mesh_obj in [section, te]:
        #     for key in list(mesh_obj.cell_data.keys()):
        #         arr = mesh_obj.cell_data[key]
        #         if len(arr) == mesh_obj.n_cells:
        #             pass  # keep as cell data
        #         elif len(arr) == mesh_obj.n_points:
        #             mesh_obj.point_data[key] = arr
        #             del mesh_obj.cell_data[key]
        #             logger.warning(f"Moved '{key}' from cell_data to point_data (length {len(arr)} matches n_points)")
        #         else:
        #             del mesh_obj.cell_data[key]
        #             logger.warning(f"Removed invalid cell data '{key}' (length {len(arr)}, n_cells {mesh_obj.n_cells}, n_points {mesh_obj.n_points})")
        airfoil = pv.merge([section, te])
        # airfoil.clean()
        # Clean merged mesh cell data
        # for key in list(airfoil.cell_data.keys()):
        #     arr = airfoil.cell_data[key]
        #     if len(arr) == airfoil.n_cells:
        #         pass
        #     elif len(arr) == airfoil.n_points:
        #         airfoil.point_data[key] = arr
        #         del airfoil.cell_data[key]
        #         logger.warning(
        #             f"Moved '{key}' from cell_data to point_data in merged mesh"
        #         )
        #     else:
        #         del airfoil.cell_data[key]
        #         logger.warning(f"Removed invalid cell data '{key}' from merged mesh")
    else:
        airfoil = section
    points_2d = airfoil.points[:, :2].tolist()
    logger.info(f"Extracted {len(points_2d)} airfoil points")
...
def define_skins_and_webs(
    ...
    skin_thickness_keys = sorted(
        airfoil_thicknesses, key=lambda k: int(re.search(r"ply_(\d+)", k).group(1))
    )
    skins = {}
    ...
    web_names_in_order = (
        ...
        if orientations
        else [f"web{i + 1}" for i in range(len(web_data))]
    )

    for idx, (web_points_2d, _) in enumerate(web_data):
        web_name = (
            web_names_in_order[idx]
            if idx < len(web_names_in_order)
            else f"web{idx + 1}"
        )
        ...
        ply_thickness_keys = sorted(
            thicknesses, key=lambda k: int(re.search(r"ply_(\d+)", k).group(1))
        )
        plies = []
        ...
        if orientations and web_name in orientations:
            ...
            logger.info(
                f"Web {web_name}: original {orig} → final_orientation {final_orientation} (after global transform + twist)"
            )
        else:
            normal_ref = [1 if idx % 2 == 0 else -1, 0]
            final_orientation = [normal_ref[0], normal_ref[1], 0.0]
            logger.warning(
                f"No orientation for {web_name}, using fallback {final_orientation}"
            )
        ...
        logger.info(
            f"Defined web {web_name}: {len(web_points_2d)} points, {len(plies)} plies"
        )
...
def log_thicknesses(skins: dict, web_definition: dict) -> None:
    ...
    for skin_name, skin in skins.items():
        if skin.thickness.array:
            logger.info(
                f"Skin {skin_name}: min {min(skin.thickness.array):.3f}, max {max(skin.thickness.array):.3f}"
            )
    for web_name, web in web_definition.items():
        for i, ply in enumerate(web.plies):
            if ply.thickness.array:
                logger.info(
                    f"Web {web_name} ply {i}: min {min(ply.thickness.array):.3f}, max {max(ply.thickness.array):.3f}"
                )
