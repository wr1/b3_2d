...
def test_example_programmatic_usage():
    ...
    examples_to_test = ["examples.mesh_example", "examples.mesh_3web_example"]
...
@pytest.mark.skipif(
    not (
        Path("examples/draped.vtk").exists()
        or Path("examples/draped_3web.vtk").exists()
    ),
...
def test_full_example_pipeline(tmp_path):
    ...
    for vtp_file in available_files:
        ...
        assert len(successful) > 0, (
            f"At least one section should process successfully for {vtp_file}"
        )

        print(
            f"✓ Full pipeline test passed for {vtp_file}: {len(successful)}/{len(results)} successful"
        )
